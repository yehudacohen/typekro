...
async function demonstrateDirectModeAlchemyIntegration() {
  ...
  await alchemyScope.run(async () => {
    ...
    const kubernetesResources = Object.entries(alchemyState).filter(
      ([_id, state]: [string, AlchemyResourceState]) => state.kind?.startsWith('kubernetes::')
    );
    ...
    kubernetesResources.forEach(
      ([resourceId, resourceState]: [string, AlchemyResourceState], index) => {
        ...
        console.log(
          `      Deployed At: ${new Date(resourceState.deployedAt as string).toISOString()}`
        );
        console.log('');
      }
    );
    ...
    const resourceTypes = [
      ...new Set(
        kubernetesResources.map(([_id, state]: [string, AlchemyResourceState]) => state.kind)
      ),
    ];
    ...
    deploymentResources.forEach(([id, state]: [string, AlchemyResourceState]) => {
      console.log(
        `     - ${id}: ${state.resource?.metadata?.name} (${state.ready ? 'Ready' : 'Pending'})`
      );
    });
    ...
    console.log(
      `   • Total Alchemy resources: ${kubernetesResources.length} (one per K8s resource)`
    );
    console.log('');
    ...
    console.log(
      '✅ Individual Kubernetes resources are registered as separate Alchemy resource types'
    );
...
async function demonstrateErrorHandling() {
  ...
  await alchemyScope.run(async () => {
    try {
      ...
      const resources = Object.entries(state).filter(([_id, s]: [string, AlchemyResourceState]) =>
        s.kind?.startsWith('kubernetes::')
      );

      console.log(`   Resources in Alchemy state: ${resources.length}`);
      resources.forEach(([id, resourceState]: [string, AlchemyResourceState]) => {
        console.log(
          `   - ${id}: ${resourceState.kind} (${resourceState.ready ? 'Ready' : 'Failed/Pending'})`
        );
      });
    } catch (error) {
      console.log(
        `❌ Deployment failed as expected: ${error instanceof Error ? error.message : String(error)}`
      );
...
export { demonstrateDirectModeAlchemyIntegration, demonstrateErrorHandling };
