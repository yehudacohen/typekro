...
| `src/bootstrap.ts` | The one registration `passwordLogin: false` lets through: the `initialUser`'s own, matched against its email from the env and its password, re-read from the projected Secret file on every attempt. |
