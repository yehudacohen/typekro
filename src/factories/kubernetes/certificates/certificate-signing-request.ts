...
const certificateSigningRequestReadinessEvaluator = registerPortableReadinessEvaluator(
  'typekro.readiness.kubernetes.certificate-signing-request',
  '1',
  (liveResource: V1CertificateSigningRequest) => {
    ...
    const approved = conditions.find((condition) => condition.type === 'Approved');
    const denied = conditions.find((condition) => condition.type === 'Denied');
...
export function certificateSigningRequest(
  ...
  return createResource(
    ...
    { scope: 'cluster' }
  ).withReadinessEvaluator(certificateSigningRequestReadinessEvaluator);
}
