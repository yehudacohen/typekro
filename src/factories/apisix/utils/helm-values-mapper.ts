  ...
  const helmValues: Partial<APISixHelmValues> = {
    ...
    ...(config.ingressController && {
      ingressController: {
        ...
        ...(config.ingressController.enabled !== undefined && {
          enabled: config.ingressController.enabled,
        }),
        ...(config.ingressController.image && { image: config.ingressController.image }),
        ...(config.ingressController.resources && {
          resources: config.ingressController.resources,
        }),
        ...(config.ingressController.nodeSelector && {
          nodeSelector: config.ingressController.nodeSelector,
        }),
        ...(config.ingressController.tolerations && {
          tolerations: config.ingressController.tolerations,
        }),
        ...(config.ingressController.affinity && { affinity: config.ingressController.affinity }),
        ...(config.ingressController.securityContext && {
          securityContext: config.ingressController.securityContext,
        }),
        ...(config.ingressController.containerSecurityContext && {
          containerSecurityContext: config.ingressController.containerSecurityContext,
        }),
        ...(config.ingressController.extraArgs && {
          extraArgs: config.ingressController.extraArgs,
        }),
...
export function validateAPISixHelmValues(values: APISixHelmValues): string[] {
  ...
  if (!values.gateway?.http?.enabled && !values.gateway?.https?.enabled) {
    warnings.push(
      'Both HTTP and HTTPS are disabled on the gateway. Consider enabling at least one.'
    );
  }
  ...
  if (!values.apisix?.resources?.requests) {
    warnings.push(
      'No resource requests specified for APISix. Consider setting CPU and memory requests for better scheduling.'
    );
  }
  ...
  if (
    ...
    !values.ingressController?.config?.kubernetes?.ingressClass
  ) {
    warnings.push(
      'No ingress class specified. Consider setting ingressClass for proper ingress handling.'
    );
  }
