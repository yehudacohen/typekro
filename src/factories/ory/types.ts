...
const jobValuesSchema = type({
  ...
  'spec?': { 'backoffLimit?': 'number.integer' },
});
...
const oryHydraMaesterChartValuesSchema = type({
  ...
  'imagePullSecrets?': yamlObjectArraySchema,
  'priorityClassName?': 'string',
  'adminService?': { 'name?': 'string', 'port?': 'number.integer', 'endpoint?': 'string', 'scheme?': 'string', 'tlsTrustStorePath?': 'string', 'insecureSkipVerify?': 'boolean' },
...
const oauth2ClientConfigSchema = type({
  ...
  spec: {
    ...
    'clientName?': 'string',
    'clientSecretExpiresAt?': 'number.integer',
    'clientUri?': 'string',
    'contacts?': 'string[]',
...
export const OryIdentityStackConfigSchema = type({
  ...
  'keto?': {
    'dsn?': oryValueSourceSchema,
    'namespaces?': type({ id: 'number.integer', name: 'string' }).array(),
    'replicaCount?': 'number.integer',
    'resources?': oryResourceRequirementsSchema,
    'serviceMonitor?': oryServiceMonitorSchema,
