 ...
 * ```
 */
export function makeWebAppWithProcessing(
  options: WebAppWithProcessingBuildOptions = {}
): CallableComposition<WebAppWithProcessingConfig, WebAppWithProcessingStatus> {
  if (containsKubernetesRefs(options)) {
    throw new Error(
      'makeWebAppWithProcessing: valkeyOperator is a build-time option and must be concrete. ' +
  ...
  if (options.valkeyOperator?.shared === false) {
    throw new Error(
      'makeWebAppWithProcessing: valkeyOperator.shared=false is no longer supported because the webapp consumes one cluster singleton. ' +
