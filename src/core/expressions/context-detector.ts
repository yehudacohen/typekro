...
export class ExpressionContextDetector {
  ...
  detectContextFromFunction(
    ...
    if (
      functionName.startsWith('simple') ||
      functionName.includes('deployment') ||
      functionName.includes('service') ||
      functionName.includes('configMap')
    ) {
  ...
  private analyzeContextualHints(config: ContextDetectionConfig): ContextualAnalysis {
    ...
    if (config.functionContext) {
      ...
      if (
        config.functionContext.includes('resource') ||
        config.functionContext.includes('Resource')
      ) {
    ...
    if (config.callStack) {
      for (const frame of config.callStack) {
        ...
        if (
          frame.includes('simple') &&
          (frame.includes('deployment') || frame.includes('service'))
        ) {
  ...
  private combineAnalyses(
    ...
    if (contextual.functionContext) {
      if (
        contextual.functionContext.includes('status') ||
        contextual.functionContext.includes('Status')
      ) {
      ...
      if (
        contextual.functionContext.includes('simple') ||
        contextual.functionContext.includes('deployment') ||
        contextual.functionContext.includes('service')
      ) {
