...
export class FieldHydrationExpressionProcessor {
  ...
  processStatusExpressions<TSpec extends Record<string, any>, TStatus>(
    ...
    try {
      ...
      const hydrationOrder =
        this.options.hydrationStrategy.calculateHydrationOrder(fieldDependencies);

      // Extract all resource and schema references
      const { allResourceReferences, allSchemaReferences } =
        this.categorizeAllReferences(fieldDependencies);
  ...
  integrateWithFieldHydrationStrategy(
    ...
    while (processed.size < hydrationOrder.length) {
      ...
      for (const fieldName of hydrationOrder) {
        ...
        const resourceDeps = fieldDeps
          .filter((dep) => dep.type === 'resource')
          .map((dep) => dep.resourceId);
  ...
  private extractKubernetesRefsFromValue(value: unknown): KubernetesRef<unknown>[] {
   ...
   */
  private categorizeAllReferences(fieldDependencies: Map<string, ResourceReference[]>): {
    allResourceReferences: ResourceReference[];
    allSchemaReferences: ResourceReference[];
  } {
