...
export class ContextAwareCelGenerator {
  ...
  private generateResourceReference(
    ...
    if (kubernetesRefs.length === 1) {
      ...
      debugInfo.transformationSteps.push(
        `Single resource reference: ${ref.resourceId}.${ref.fieldPath}`
      );
  ...
  private generateConditionalCheck(
    ...
    if (kubernetesRefs.length === 1) {
      ...
      debugInfo.transformationSteps.push(
        `Single conditional reference: ${ref.resourceId}.${ref.fieldPath}`
      );
      ...
      debugInfo.transformationSteps.push(
        `Multiple conditional references: ${kubernetesRefs.length}`
      );
  ...
  private generateReadinessCheck(
    ...
    if (kubernetesRefs.length === 1) {
      ...
      debugInfo.transformationSteps.push(
        `Single readiness reference: ${ref.resourceId}.${ref.fieldPath}`
      );
  ...
  private generateTemplateInterpolation(
    ...
    const expression = interpolations.length === 1 ? interpolations[0] : interpolations.join(' + ');
