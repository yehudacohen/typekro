  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule({
      ...
      validate: (_expression, kubernetesRefs, _context, _config) => {
        const statusRefs = kubernetesRefs.filter(
          (ref) => ref.fieldPath.includes('status') || ref.resourceId === '__schema__'
        );
    ...
    this.addRule({
      ...
      severity: 'warning',
      validate: (_expression, kubernetesRefs, _context, _config) => {
        const inappropriateRefs = kubernetesRefs.filter(
          (ref) =>
    ...
    this.addRule({
      ...
      validate: (expression, kubernetesRefs, _context, _config) => {
        ...
        const hasBooleanKeywords = /ready|available|enabled|disabled|true|false/i.test(
          expressionString
        );
        const referencesReadyFields = kubernetesRefs.some(
          (ref) =>
    ...
    this.addRule({
      ...
      validate: (expression, _kubernetesRefs, _context, config) => {
        ...
        const expressionString = String(expression);
        if (
          expressionString.includes('.hasOwnProperty') ||
    ...
    this.addRule({
      ...
      validate: (expression, kubernetesRefs, _context, config) => {
        ...
        return {
          valid: true,
          message: `Expression is compatible with ${config.factoryType} factory`,
        };
      },
    });
  ...
  private getApplicableRules(
    ...
    if (config.customRules) {
      applicableRules.push(
        ...config.customRules.filter((rule) => rule.applicableContexts.includes(context))
      );
    }
