...
import type { CelGenerationConfig, ContextAwareCelGenerator } from './context-aware-generator.js';
import type {
...
export class ExpressionContextSwitcher {
    ...
    try {
      ...
      if (config.validateContextSwitches) {
        for (const [context, points] of contextGroups) {
          ...
          if (!report.valid) {
            warnings.push(
              `Context ${context} validation failed: ${report.errors.map((e) => e.message).join(', ')}`
            );
          }
        }
      }
  ...
  private findContextSwitchPoints(
    ...
    if (Array.isArray(expression)) {
      ...
      this.analyzeStringExpression(expression, currentContext, path, depth, config, switchPoints);
    }
  }
    ...
    return [
...

      {
        ...
        condition: (expression, currentContext) => {
          ...
          return (
            /ready|available|healthy|running/i.test(expressionString) &&
...

      {
        ...
        condition: (expression, currentContext) => {
          ...
          if (isKubernetesRef(expression)) {
            ...
            return String(expression).includes('status') && currentContext !== 'status-builder';
          }

          return (
            refs.some((ref) => ref.fieldPath.includes('status')) &&
