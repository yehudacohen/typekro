...
export function separateStatusFields(statusMappings: Record<string, any>): {
  ...
  for (const [fieldName, fieldValue] of Object.entries(statusMappings)) {
    if (typeof fieldValue === 'object' && fieldValue !== null && !Array.isArray(fieldValue) && !isKubernetesRef(fieldValue) && !isCelExpression(fieldValue)) {
...
export function validateStatusCelExpressions(
  ...
  const resourceIds = new Set(Object.values(resources).map(r => r.id).filter(Boolean));
 ...
 */
export function validateResourceIds(resources: Record<string, KubernetesResource>): CelValidationResult {
  const errors: CelValidationError[] = [];
