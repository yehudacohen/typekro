...
describeOrSkip('End-to-End Helm Integration', () => {
  ...
  it('should deploy Flux Helm Controller and HelmRelease resources via Direct Factory', async () => {
    console.log('🚀 Starting end-to-end Helm integration test with real cluster deployment...');
    console.log('🎯 This test verifies that yamlFile and yamlDirectory factories work end-to-end with proper readiness evaluation');
    ...
    console.log('📊 Step 1: Creating comprehensive resource graph with Helm Controller and HelmReleases...');
    ...
    console.log('   - TypeKro orchestrates both YAML closures and Enhanced<> resources seamlessly');
    ...
    try {
      ...
      console.log('📦 HelmRepository spec:', JSON.stringify((helmRepo.body as any).spec, null, 2));
    } catch (error) {
    ...
    try {
      ...
      console.log('📊 NGINX HelmRelease spec:', JSON.stringify((nginxHelmRelease.body as any).spec, null, 2));
      ...
      console.log('📊 Redis HelmRelease spec:', JSON.stringify((redisHelmRelease.body as any).spec, null, 2));
    ...
    console.log('✅ yamlFile() factory: Downloaded and applied complete Flux system from GitHub HTTP URL');
    console.log('✅ Enhanced<> resources: HelmRepository and HelmRelease resources deployed with full type safety');
    console.log('✅ Readiness evaluation: Proper waiting for all resources to be ready (no waitForReady: false hacks)');
    console.log('✅ Conflict handling: skipIfExists strategy prevents conflicts with existing resources');
    console.log('✅ GitOps workflow: Complete GitOps deployment from TypeScript to running Helm charts');
    console.log('✅ Parallel execution: YAML closures and Enhanced<> resources deployed simultaneously');
    console.log('✅ End-to-end validation: Verified actual Kubernetes resources are created and functional');
    console.log('');
  ...
  it('should demonstrate Helm readiness evaluation with mock data', async () => {
    ...
    const { helmReleaseReadinessEvaluator } = await import('../../src/factories/helm/readiness-evaluators.js');
