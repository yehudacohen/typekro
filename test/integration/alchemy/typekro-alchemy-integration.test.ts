    ...
    describe('TypeKro values to Alchemy properties', () => {
        it('should pass TypeKro resource references to Alchemy resources', async () => {
            ...
            await alchemyScope.run(async () => {
                ...
                const configFileState = Object.values(alchemyState).find((state: any) =>
                    state.kind === 'fs::File' && state.output?.path === configFile.path
                ) as any;
                const appLogFileState = Object.values(alchemyState).find((state: any) =>
                    state.kind === 'fs::File' && state.output?.path === appLogFile.path
                ) as any;
                const configJsonFileState = Object.values(alchemyState).find((state: any) =>
                    state.kind === 'fs::File' && state.output?.path === configJsonFile.path
                ) as any;
                ...
                expect(appLogFileState?.output.content).toContain('TypeKro-Alchemy integration test started');
                ...
                console.log(`✅ Alchemy state validation passed - ${resourceIds.length} resources in state`);
                ...
                console.log(`   - Config JSON file: ${configJsonFileState?.id} (${configJsonFileState?.status})`);
    ...
    describe('Alchemy values to TypeKro properties', () => {
        it('should demonstrate how Alchemy resource outputs become TypeKro inputs', async () => {
            ...
            await alchemyScope.run(async () => {
                ...
                // Verify that our File resources are registered in alchemy state
                const dbConfigState = Object.values(inputTestState).find((state: any) =>
                    state.kind === 'fs::File' && state.output?.path === alchemyDatabase.configFile.path
                ) as any;
                const envConfigState = Object.values(inputTestState).find((state: any) =>
                ...
                (schema) => ({
                    webapp: simpleDeployment({
                        ...
                        env: {
                            ...
                            DATABASE_URL: Cel.expr('postgresql://', schema.spec.databaseEndpoint, ':', Cel.string(schema.spec.databasePort), '/webapp'),
                        },
                    }),
            ...
            expect(yaml).toContain('value: ${postgresql://schema.spec.databaseEndpoint:string(schema.spec.databasePort)/webapp}');
...
`.trim()
                ...
                // Verify that our File resources are registered in alchemy state
                const dbConfigState = Object.values(fullstackState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === alchemyDatabase.configFile.path
                ) as any;
                const storageConfigState = Object.values(fullstackState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === alchemyBucket.configFile.path
                ) as any;
                const appConfigState = Object.values(fullstackState).find((state: any) =>
                ...
                console.log(`   - DB config: ${dbConfigState?.id} (${dbConfigState?.status})`);
                console.log(`   - Storage config: ${storageConfigState?.id} (${storageConfigState?.status})`);
    ...
    describe('Factory integration with Alchemy', () => {
        ...
        it('should show how deployment handles mixed Alchemy and TypeKro resources', async () => {
            ...
            await alchemyScope.run(async () => {
                ...
                // Verify that our File resources are registered in alchemy state
                const configState = Object.values(externalDbState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === externalDatabase.configFile.path
                ) as any;
                const credentialsState = Object.values(externalDbState).find((state: any) =>
...
`.trim()
                ...
                // Verify that our File resources are registered in alchemy state
                const vpcConfigState = Object.values(awsState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === vpc.configFile.path
                ) as any;
                const dbConfigState = Object.values(awsState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === database.configFile.path
                ) as any;
                const s3ConfigState = Object.values(awsState).find((state: any) =>
                    state.kind === 'fs::File' && state.output.path === s3Bucket.configFile.path
                ) as any;
                const terraformConfigState = Object.values(awsState).find((state: any) =>
                ...
                console.log(`   - Terraform config: ${terraformConfigState?.id} (${terraformConfigState?.status})`);
            });
