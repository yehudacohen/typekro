...
describeOrSkip('AlchemyDeploymentStrategy Error Handling', () => {
    ...
    beforeAll(async () => {
        ...
        mockDeploymentEngine = {
            ...
            deployResource: async (resource: any, _options: any) => {
                ...
                throw new Error(`Simulated deployment failure for ${resource.kind}/${resource.metadata?.name || resource.name}`);
            },
    ...
    describe('Individual resource failure scenarios', () => {
        it('should continue processing remaining resources when individual resources fail', async () => {
            ...
            await alchemyScope.run(async () => {
                ...
                console.log(`✅ Continued processing despite failures - Alchemy state has ${Object.keys(alchemyState).length} resources`);
            });
        }, TEST_TIMEOUT);

        it('should collect all errors and include them in the final DeploymentResult', async () => {
        ...
        it('should set deployment status to partial when some resources succeed and others fail', async () => {
    ...
    describe('Validation error scenarios', () => {
        it('should handle Alchemy scope validation failures', async () => {
            const baseStrategy = new DirectDeploymentStrategy(
            ...
            await expect(alchemyStrategy.deploy({
