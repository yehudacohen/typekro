...
describe('Cilium Networking CRD Factories', () => {
  describe('CiliumNetworkPolicy Factory', () => {
    ...
    it('should throw error if name is missing', () => {
      const resource = {
        apiVersion: 'cilium.io/v2',
        kind: 'CiliumNetworkPolicy',
        metadata: {} as unknown as { name: string }, // Force empty metadata to test validation
        spec: {
          endpointSelector: {
            matchLabels: { app: 'test' },
          },
    ...
    it('should throw error if ingress rule has no source', () => {
      ...
      expect(() => ciliumNetworkPolicy(resource)).toThrow(
        'CiliumNetworkPolicy ingress rule 0 must specify at least one source'
      );
    });
    ...
    it('should throw error if egress rule has no destination', () => {
      ...
      expect(() => ciliumNetworkPolicy(resource)).toThrow(
        'CiliumNetworkPolicy egress rule 0 must specify at least one destination'
      );
    });
  });
  ...
  describe('CiliumClusterwideNetworkPolicy Factory', () => {
    ...
    it('should throw error if name is missing', () => {
      ...
      expect(() => ciliumClusterwideNetworkPolicy(resource)).toThrow(
  ...
  describe('CiliumClusterwideNetworkPolicy Readiness Evaluator', () => {
    ...
    it('should return ready when Ready condition is True', () => {
      ...
      expect(result.message).toBe(
        'CiliumClusterwideNetworkPolicy is ready and applied cluster-wide'
      );
      expect(result.details?.lastTransition).toBe('2024-01-01T00:00:00Z');
    });
  ...
  describe('Factory Integration', () => {
    it('should create resources with embedded readiness evaluators', () => {
      ...
      expect(getReadinessEvaluator(networkPolicy)).toBeDefined();
      expect(getReadinessEvaluator(clusterPolicy)).toBeDefined();

      // Test that evaluators work
      const networkResult = requireReadinessEvaluator(networkPolicy)(networkPolicy);
      const clusterResult = requireReadinessEvaluator(clusterPolicy)(clusterPolicy);
