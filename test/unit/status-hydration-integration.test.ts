...
describe('Status Hydration Integration with DirectDeploymentEngine', () => {
  ...
  let mockK8sApi: Record<string, unknown> & { _resourceExists: boolean };
  ...
  beforeEach(() => {
    ...
    mockK8sApi = {
      ...
      create: async (resource: Record<string, unknown>) => resource,
      replace: async (resource: Record<string, unknown>) => resource,
      patch: async (resource: Record<string, unknown>) => resource,
  ...
  it('should integrate status hydration with custom readiness evaluation', async () => {
    ...
    (deploymentResource as unknown as Record<string, unknown>).id = 'test-deployment';
    ...
    const deployedResource = await engine.deployResource(
      deploymentResource as unknown as Parameters<typeof engine.deployResource>[0],
      options
    );
    ...
    const enhanced = deployedResource.manifest as unknown as Record<string, unknown>;
  ...
  it('should work without status hydration when disabled', async () => {
    ...
    // Add id property to the Enhanced proxy directly
    (deploymentResource as unknown as Record<string, unknown>).id = 'test-deployment-no-hydration';
    ...
    const statusKeys = Object.keys(enhanced.status as Record<string, unknown>);
  ...
  it('should handle status hydration failures gracefully', async () => {
    ...
    // Add id property to the Enhanced proxy directly
    (deploymentResource as unknown as Record<string, unknown>).id = 'test-deployment-fail';
  ...
  it('should eliminate duplicate API calls by reusing readiness check data', async () => {
    ...
    // NOTE: In the new API, methods return objects directly (no .body wrapper)
    mockK8sApi.read = async (resource: Record<string, unknown>) => {
      apiCallCount++;
      const metadata = resource.metadata as Record<string, unknown>;
    ...
    mockK8sApi.create = async (resource: Record<string, unknown>) => {
      apiCallCount++;
    ...
    // Add id property to the Enhanced proxy directly
    (deploymentResource as unknown as Record<string, unknown>).id = 'test-deployment-efficient';
