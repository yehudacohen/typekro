...
describe('Complex Expressions and Edge Cases', () => {
  ...
  describe('Array and Object Method Calls', () => {
    it('should handle simple array method expressions', () => {
      // Simple binary predicates are convertible to CEL
      const simpleExpressions = ['service.spec.ports.find(p => p.name === "http")?.port'];
    ...
    it('should reject complex array method chains that cannot be converted to CEL', () => {
      // Complex predicates (LogicalExpression, nested calls, chained methods) cannot
      // be automatically converted to CEL — they require Cel.expr() instead
    ...
    it('should handle convertible chained array methods', () => {
      // Some chained methods with simple binary predicates can be converted
      const convertibleExpressions = [
      ...
      for (const expr of convertibleExpressions) {
        const result = analyzer.analyzeExpression(expr, mockContext);
        expect(result.valid).toBe(true);
      }
    });

    it('should reject filter with LogicalExpression predicate chained to map', () => {
      // LogicalExpression (&&) in filter predicate is not directly supported;
      // only simple BinaryExpression and MemberExpression predicates are convertible
    ...
    it('should convert filter with simple binary predicate chained to map', () => {
      const result = analyzer.analyzeExpression(
        'service.spec.ports.filter(p => p.protocol === "TCP").map(p => p.targetPort)',
        mockContext
      );
      expect(result.valid).toBe(true);
    });

    it('should convert find with nested some call', () => {
      const result = analyzer.analyzeExpression(
    ...
    it('should convert deeply chained find expressions', () => {
      const result = analyzer.analyzeExpression(
    ...
    it('should reject find predicates with non-binary logical conditions', () => {
      // A predicate like `c.type === "Available" && c.status === "True" && c.lastTransitionTime`
      // has a truthy check (not a binary comparison) as part of the LogicalExpression,
      // which cannot be converted to CEL
      const result = analyzer.analyzeExpression(
  ...
  describe('Performance with Complex Expressions', () => {
    ...
    it('should handle expressions with many nested levels', () => {
      const deeplyNestedExpression =
        'deployment.spec.template.spec.containers[0].env.find(e => e.name === "CONFIG").valueFrom.configMapKeyRef.key';
