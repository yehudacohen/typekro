...
describe('MagicAssignableAnalyzer', () => {
  ...
  describe('analyzeMagicAssignableShape', () => {
    ...
    it('should detect and convert KubernetesRef objects in shape fields', () => {
      ...
      const serviceRef = createMockKubernetesRef<string>(
        'service',
      ...
      expect(
        (result.processedShape.replicas as unknown as Record<symbol, unknown>)[CEL_EXPRESSION_BRAND]
      ).toBe(true);
      expect(
        (result.processedShape.url as unknown as Record<symbol, unknown>)[CEL_EXPRESSION_BRAND]
      ).toBe(true);
    });
    ...
    it('should handle nested object shapes', () => {
      const analyzer = createAnalyzer();
      const context = createContext();
      const deploymentRef = createMockKubernetesRef<boolean>(
        'deployment',
      ...
      expect(
        (result.processedShape.metadata.ready as unknown as Record<symbol, unknown>)[
