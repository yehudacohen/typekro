...
import {
  ExpressionCache,
  globalExpressionCache,
} from '../../../src/core/expressions/analysis/cache.js';
import type { CelExpression } from '../../../src/core/types/common.js';
import type { Enhanced } from '../../../src/core/types/kubernetes.js';

/** Create a minimal typed placeholder for `availableReferences` entries in cache tests. */
function emptyRef(): Enhanced<Record<string, unknown>, Record<string, unknown>> {
  return {} as unknown as Enhanced<Record<string, unknown>, Record<string, unknown>>;
}

/** Create a minimal CelExpression placeholder for cache entry testing. */
function celExpr(expression: string): CelExpression {
  return { expression } as unknown as CelExpression;
}
...
describe('Performance Optimization and Caching', () => {
  ...
  describe('Expression Caching', () => {
    it('should cache conversion results', () => {
      ...
      const context: AnalysisContext = {
        type: 'status',
        availableReferences: { deployment: emptyRef() },
        factoryType: 'direct',
      };
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          smallCache.set(expression, context, {
            valid: true,
            celExpression: celExpr(`resources.deployment.status.field${i}`),
            dependencies: [],
            sourceMap: [],
            errors: [],
            warnings: [],
    ...
    it('should enforce memory limits', () => {
      ...
      try {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const largeResult = {
            valid: true,
            celExpression: celExpr('x'.repeat(1000)), // Large expression
            dependencies: [],
            sourceMap: [],
            errors: [],
            requiresConversion: true,
  ...
  describe('Cache Key Generation', () => {
    it('should generate different keys for different contexts', () => {
      ...
      const context2: AnalysisContext = {
        type: 'status',
        availableReferences: { service: emptyRef() },
        factoryType: 'direct',
      };
  ...
  describe('Performance Benchmarks', () => {
    it('should demonstrate cache performance benefits', () => {
      ...
      console.log(
        `Cache performance: ${stats.hitRatio * 100}% hit ratio, ${totalTime}ms for 500 operations`
      );
    });
  });
});
