...
**`factory('direct')` limitation (live-verified against a real cluster).** The metadata/spec-anchored fields above (`clickhouse.host`/`nativeUrl`/`httpUrl`/`clusterName`, `keeper.*`, `installation.name`/`namespace`) are built from a raw `Cel.expr("...")` string — required, because plain proxy access on `.metadata.*` is misclassified as schema-derived and dropped from KRO status entirely. That workaround fixes `factory('kro')` reachability (confirmed: the live `ClickHouseCluster` CR's status carries the full contract after a real deploy) but is opaque to `factory('direct')`'s live-status re-execution, which has no CEL interpreter and can only hydrate fields it evaluates as plain JS against the live resource. So **in direct mode only**, those fields come back as unresolved `CelExpression` markers (check with `isCelExpression(value)` from `typekro`) rather than concrete strings — `ready`/`phase`/`installation.endpoint`/`hostsCount`/`hostsCompletedCount` (built from plain `clickhouse.status.*` access) hydrate correctly in both modes. This is a narrow typekro gap, tracked upstream ([typekro#94](https://github.com/yehudacohen/typekro/issues/94)) — not something this factory can paper over. **UPDATE:** the root cause is fixed in [typekro#97](https://github.com/yehudacohen/typekro/pull/97) (resource `.metadata.*` reads inside status builders no longer silently degrade to schema refs); once that lands and this factory bumps its typekro dependency, the raw `Cel.expr` workaround can be dropped in favor of natural proxy syntax, and these fields will hydrate correctly in BOTH modes. `factory('kro')` deployments always get the full live contract; a `factory('direct')` caller needing the connection string can build it from `spec.name`/`spec.namespace` (the same naming rule, known synchronously).
