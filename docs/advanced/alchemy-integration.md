...
- **The generated CRD is LEFT for out-of-band GC.** Unlike the imperative `deleteInstance` path (which issues a best-effort CRD delete as its last step), the alchemy path does **not** delete the composition's generated CRD on teardown — the CRD is cluster-scoped, KRO never deletes it (`allowCRDDeletion=false`), and its apiextensions cleanup finalizer can stall for minutes ([kro#1171](https://github.com/kro-run/kro/issues/1171)). It is harmless with zero instances; garbage-collect it out-of-band if desired (`kubectl delete crd <plural>.<group>`).
