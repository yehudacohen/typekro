...
- A composition MAY create a `Namespace` resource for the same namespace that contains the KRO instance (the self-owned bootstrap pattern). TypeKro handles it automatically: TypeKro **never emits a `Namespace` into RGD YAML** — every owned Namespace is **hoisted out of the RGD graph** and applied as a **sibling** (created deps-first, deleted after the RGD via an empty-gated teardown), so deleting the instance can never strand its finalizer by garbage-collecting its own namespace. Upgrading a *pre-hoist* deployment (whose Namespace was a KRO graph child) is **not** auto-migrated and fails closed on deploy; see [Migration](/advanced/migration#upgrading-from-a-pre-hoist-typekro-release-kro).
...
- When the composition creates its own Namespace, give it a fresh per-run UUID-based name, call `assertTestNamespaceAbsent()` before deployment, then call `captureTestNamespaceLease()` immediately after successful creation and retain that lease through teardown. Absence alone does not reserve a fixed name. Never reacquire a lease during cleanup: if creation or lease capture is uncertain, retain the namespace and fail the test rather than risk adopting a same-name replacement.
- Strict namespace cleanup submits a UID-preconditioned Namespace deletion and only deletes PVCs captured from the owned namespace, using each PVC's immutable UID as a deletion precondition.
...
14. **Adopting a test namespace by name** — `ensureNamespaceExists()` is unsuitable when cleanup will delete the Namespace. Use `createTestNamespace()` and retain its UID lease, or prove a composition-owned name absent before deploy and capture its lease before cleanup.
...
27. **Manually deleting RGDs/CRDs/namespaces in tests** — Use `factory.deleteInstance()` which handles the full cleanup graph. Manual deletion causes zombie instances and stale CRDs.
28. **K8s API error format** — `@kubernetes/client-node` `ApiException` uses `.code` (not `.statusCode`) for HTTP status. All catch blocks must check: `error.statusCode ?? error.code ?? error.body?.code`.
29. **Operator-required labels** — Some operators (e.g., Hyperspike Valkey) panic on nil label maps. Always include standard `app.kubernetes.io/*` labels on resources where the operator copies labels to child resources.
30. **Using `createResource` for standard K8s types** — Always use the built-in factories (`configMap()`, `secret()`, `deployment()`, etc.) instead of `createResource` directly for standard K8s resources. The built-in factories include readiness evaluators that the deployment engine requires. Without a readiness evaluator, `waitForReady: true` will fail with "No readiness evaluator found."
31. **Composition functions must be side-effect-free beyond resource creation** — During KRO schema generation, the framework re-executes the composition function with a synthetic spec (optional fields set to `undefined`, required fields set to a sentinel) to detect `??` defaults that reference imported constants and to detect ternary-controlled optional sections. The framework ALSO re-executes inside `processCompositionBodyAnalysis` with a hybrid schema proxy to capture resources from branches the proxy run didn't take (e.g., `if (!spec.x) { createResource(...) }`). This happens automatically whenever `toYaml()` is called or an RGD is deployed. The re-execution runs inside a temporary, isolated composition context so resource registrations are contained — but **any other side effects fire a second time**:
...
33. **Compositions should read as plain TypeScript — don't reach for explicit `Cel.*` helpers** — The framework's composition analyzer handles JS-to-CEL conversion for most common patterns:
