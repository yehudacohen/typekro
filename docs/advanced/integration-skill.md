...
52. **Status leaves that project from no resource** — KRO requires every status field to refer to a RESOURCE in the graph (`instance status field must refer to a resource`), and its status CEL environment has no `schema` identifier, so a leaf that references no resource is left unset and the declared status schema promises a field the custom resource never carries. TypeKro hydrates those leaves client-side in `getStatus()`, so the author sees the value and `kubectl`, other controllers, and `externalRef` consumers do not. Two shapes qualify. **Literals** — `true`, `5432`, `'http'`, `['web', 'websecure']`; a reference-free CEL expression is not an escape, `` Cel.expr<string>`'running'` `` is dropped the same way. **Bare `schema.spec.*` echoes** — `version: spec.version`, `` url: `http://${spec.host}` ``; these look resolvable because the CR does hold the value in its own spec, but a status leaf is a projection and there is no resource here to project from. A `schema.spec.*` *inside* an expression that also references a resource is fine — `` url: `http://${service.metadata.name}.${spec.namespace}.svc` `` — because the resource supplies the dependency KRO requires. Watch for silent degradation too: `conditions?.some(c => ...) || false` cannot be expressed in CEL, so the analyzer collapses the leaf to the `|| false` fallback and the field ends up absent rather than `false`. Project the value through a resource the graph owns (a ConfigMap the composition creates works: `mode: myConfigMap.data.mode`) or drop the field. Serialization warns by default; set `allowLiteralStatus: false` on the composition options or on `factory('kro', { ... })` to make it an error. Direct mode is unaffected.
