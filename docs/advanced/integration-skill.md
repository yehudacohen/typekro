...
52. **Literal status leaves** — KRO fills an instance's `status` only from expressions it can resolve against the graph's own resources, so a leaf that is a bare literal (`true`, `5432`, `'http'`, `['web', 'websecure']`) is left unset and the declared status schema promises a field the custom resource never carries. TypeKro hydrates those leaves client-side in `getStatus()`, so the author sees the value and `kubectl`, other controllers, and `externalRef` consumers do not. A reference-free CEL expression is not an escape — `` Cel.expr<string>`'running'` `` is dropped the same way. Watch for silent degradation too: `conditions?.some(c => ...) || false` cannot be expressed in CEL, so the analyzer collapses the leaf to the `|| false` fallback and the field ends up absent rather than `false`. Project the value from a resource the graph owns (a ConfigMap the composition creates works: `mode: myConfigMap.data.mode`) or drop the field. Serialization warns by default; set `allowLiteralStatus: false` on the composition options or on `factory('kro', { ... })` to make it an error. Direct mode is unaffected.
