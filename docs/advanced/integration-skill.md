...
- A composition MAY create a `Namespace` resource for the same namespace that contains the KRO instance (the self-owned bootstrap pattern). TypeKro handles it automatically: it **hoists that owned Namespace out of the RGD graph** and emits it as a retained resource created deps-first, so deleting the instance can never strand its finalizer by garbage-collecting its own namespace. (This is a structural hoist — the Namespace's `metadata.name` must be exactly the schema namespace expression; anything unprovable fails closed and the original safety guard still fires.) Upgrading a *pre-hoist* deployment needs the one-time ownership transfer documented in [Migration](/advanced/migration#upgrading-a-self-owned-namespace-bootstrap-kro).
